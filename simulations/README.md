...
| `ecdna_spatial_niches_100div.html` | **Spatial clonal expansion from a single founder.** A tumour section full of host cells with no ecDNA; one ecDNA+ founder cell is seeded in each of two niches (A selected, B neutral). Each division, cells compete locally for space (fitness-weighted, with a small ecDNA carrying cost). In **niche A** the copy-number advantage makes the lineage supercritical, so the clone amplifies and **spreads outward from the point of origin**; in the neutral **niche B** the lone founder is subcritical and **drifts to extinction**. A scrubable bird's-eye map (only ecDNA+ cells drawn, coloured by copy number) plus the A-vs-B clone-size trajectory show the same founding circle taking opposite fates by location. Set selection to 0 and niche A dies too. | — |
